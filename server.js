...
app.post('/lineworks/callback', async (req, res) => {
  ...
  try {
    ...
    const pingResponse = await fetch('https://lineworks-reservation-bot.onrender.com/ping');
    const pingText = await pingResponse.text();

    if (pingResponse.ok && pingText === 'OK') {
      ...
      await axios.post(
        `https://www.worksapis.com/v1.0/bots/${BOT_ID}/users/${accountId}/messages`,
        {
          content: {
            type: 'text',
            text: '⚠️ サーバーが停止している可能性があります。管理者に連絡してください。'
          }
        },
        {
          headers: {
    ...
    console.error('[ERROR] /ping 接続確認失敗:', err.message);
  }
