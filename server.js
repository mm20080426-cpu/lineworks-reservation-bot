...
app.post('/lineworks/callback', async (req, res) => {
  ...
  if (!selectedDate) {
    ...
    const calendarSlots = getAvailableTimeSlots(selectedDate);
    if (!calendarSlots || calendarSlots.length === 0) {
      replyText = `🚫 ${selectedDate} は休診日です。別の日を選んでください。`;
    } else {
      const availableSlots = await getAvailableSlots(selectedDate); // ← Sheetsと照合した空き枠

      // calendar.json にある枠の中で、予約されていないものだけを抽出
