...
// ✅ ステートが未定義の場合でも入力内容に応じて初期化
if (!state || !state.step) {
  const trimmed = messageText.trim();
  const maybeDate = extractDate(trimmed);

  if (trimmed === '予約' || maybeDate) {
    userState.set(userId, { step: 'awaitingDate' });
    replyText = '📅 日付を入力してください（例：9/11 または 2025/9/11）';
  } else if (trimmed === 'キャンセル') {
    ...
    userState.set(userId, { step: 'menu' });
    replyText = '👋 はじめまして！以下のメニューから選んでください：\n・予約\n・キャンセル\n・一覧\n・空き';
  }

  return sendReply(userId, replyText);
}

// ✅ メニュー状態からの入力分岐（ステートが "menu" の場合）
if (state.step === 'menu') {
  const trimmed = messageText.trim();
  switch (trimmed) {
    case '予約':
    ...
    default:
      ...
      if (maybeDate) {
        ...
        replyText = '🤖 「予約」「キャンセル」「一覧」「空き」から選んでください。';
      }
  }
  return sendReply(userId, replyText);
}
...
// ✅ 空き枠選択 → 名前入力
if (state?.step === 'dateSelected' && /^\d+$/.test(messageText)) {
...
if (state?.step === 'awaitingCancelDate') {
  ...
  if (!cancelDate) {
    ...
    // 表示用：予約枠IDを除去
    const numberedList = reservationList.map((item, index) => {
      const cleaned = item.replace(/(\|?\s*予約[枠]?ID[:：]\s*.*$)/, '').trim();
...
if (state?.step === 'awaitingCancelSelection') {
  ...
  // 表示用：予約枠IDを除去
  const numberedUpdatedList = updatedList.map((item, i) => {
    const cleaned = item.replace(/(\|?\s*予約[枠]?ID[:：]\s*.*$)/, '').trim();
    return `${i + 1}. ${cleaned}`;
  });
...
if (state?.step === 'awaitingListDate') {
  ...
  const rawList = await getReservationsByDate(listDate);

  // 表示用：予約枠IDを除去
  const cleanedList = rawList.map(item =>
    item.replace(/(\|?\s*予約[枠]?ID[:：]\s*.*$)/, '').trim()
  );

  replyText =
    `📋 ${listDate} の予約一覧:\n` +
    (cleanedList.length ? cleanedList.join('\n') : '📭 予約はありません。');
