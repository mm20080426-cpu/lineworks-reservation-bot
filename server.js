...
// 🔍 デバッグログを出力
console.log('[DEBUG] 送信テンプレート:', JSON.stringify(templateMessage, null, 2));
console.log('[INFO] テンプレート送信成功');
console.error('[ERROR] テンプレート送信失敗:', error.response?.data || error.message);
...
// 📤 LINE WORKS APIに送信
  ...
  console.error('[ERROR] テンプレート送信失敗:', err.response?.data || err.message);
  return res.sendStatus(500); // 失敗時は500などにしてもOK
}
...
// ✅ 一覧フロー
  ...
  // ✅ 空きフロー
    ...
    if (!freeDate) {
      ...
      const slots = getAvailableTimeSlots(freeDate);
      if (!slots || slots.length === 0) {
        replyText = `🚫 ${freeDate} は休診日です。別の日を選んでください。`;
      } else {
