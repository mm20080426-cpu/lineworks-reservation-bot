...
if (messageText === '予約') {
 ...
 // ✅ キャンセルフロー
  ...
  if (!cancelDate) {
    ...
    replyText =
      `🕒 ${cancelDate} のキャンセルしたい時間枠を入力してください（例：10:00）\n\n📋 予約一覧:\n` +
      (Array.isArray(reservationList) ? reservationList.join('\n') : '📭 予約はありません。');
  }
}
 else if (state?.step === 'awaitingCancelTime') {
  const cancelTime = messageText;

  console.log('[DEBUG] キャンセル対象日:', state.cancelDate);
  console.log('[DEBUG] 入力された時間枠:', cancelTime);
  ...
  await new Promise(resolve => setTimeout(resolve, 500)); // ← 少し延ばしてもOK

  const updatedList = await getReservationsByDate(state.cancelDate);

  console.log('[DEBUG] 最新予約一覧取得結果:', updatedList);
