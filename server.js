...
app.post('/lineworks/callback', async (req, res) => {
 ...
 // ✅ Webhook登録状況を確認する関数
async function isWebhookRegistered(accessToken, botId) {
  try {
    const response = await axios.get(
      `https://www.worksapis.com/v1.0/bots/${botId}/webhook`,
      {
        headers: {
    ...
    return response.data?.endpoint && response.data.endpoint.includes('onrender.com');
  } catch (err) {
    console.error('[ERROR] Webhook確認失敗:', err.response?.data || err.message);
    return false;
  }
}

// ✅ Webhookを再登録する関数
async function registerWebhook(accessToken, botId, webhookUrl) {
  try {
    await axios.post(
      `https://www.worksapis.com/v1.0/bots/${botId}/webhook`,
      { endpoint: webhookUrl },
    ...
    console.log('[INFO] Webhook再登録成功:', webhookUrl);
  } catch (err) {
    console.error('[ERROR] Webhook再登録失敗:', err.response?.data || err.message);
  }
}

// ✅ 「開始」メッセージ処理（/lineworks/callback の中）
if (messageText === '開始') {
  try {
    const accessToken = await fetchAccessToken();
    const accountId = await getAccountIdFromUserId(userId, accessToken);
    const webhookUrl = 'https://lineworks-reservation-bot.onrender.com/lineworks/callback';

    // ✅ Webhook未登録なら再登録
    const registered = await isWebhookRegistered(accessToken, BOT_ID);
    if (!registered) {
      await registerWebhook(accessToken, BOT_ID, webhookUrl);
    }

    // ✅ Renderの /ping にアクセスして疎通確認
    let replyText = '';
    try {
      const pingResponse = await axios.get('https://lineworks-reservation-bot.onrender.com/ping');
      const pingText = pingResponse.data;

      if (pingResponse.status === 200 && pingText === 'OK') {
      ...
      console.error('[ERROR] /ping 接続失敗:', pingErr.message);
      replyText = '⚠️ サーバーが停止している可能性があります。管理者に連絡してください。';
    }
    ...
    console.error('[ERROR] 開始処理全体の失敗:', err.message);
  }
