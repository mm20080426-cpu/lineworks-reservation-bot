...
function extractReservationId(text) {
  const match = text.match(/予約枠ID[:：]?\s*([a-zA-Z0-9\-]+)/);
...
async function handleBotMessage(userId, messageText) {
  ...
  if (context?.step === 'awaiting_date') {
    try {
      const rawList = await getReservationsByDate(messageText); // ← normalizeDate を使わない
  ...
  if (context?.step === 'awaiting_selection') {
    ...
    console.log('[DEBUG] 抽出された日付:', selected.date);
