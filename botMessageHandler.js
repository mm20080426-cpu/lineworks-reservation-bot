...
async function handleBotMessage(userId, messageText) {
  ...
  if (context?.step === 'awaitingCancelDate') {
    ...
    const originalList = rawReservations.map(r => {
      ...
      return display; // display文字列のみ保存
    });

    cancelContext.set(userId, {
      step: 'awaitingCancelSelection',
      cancelDate: selectedDate, // ← ここが selectedDate ではなく cancelDate になっていることに注意
      originalList
    });
  ...
  if (context?.step === 'awaitingCancelSelection') {
    ...
    let { originalList, cancelDate } = context;

    console.log('[DEBUG] ステップ② cancelDate:', cancelDate);
    ...
    // 🔧 強制構造化
    originalList = ensureStructuredList(originalList, cancelDate);
    ...
    if (!selectedRaw || typeof selectedRaw !== 'object') {
      console.warn('[WARN] selectedRaw の構造が不正です:', selectedRaw);
      return '⚠️ キャンセル対象の情報が不正です。再度選択してください。';
    }

    const { reservationId, timeSlot, selectedDate } = selectedRaw;
