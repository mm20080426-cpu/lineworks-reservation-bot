...
async function handleBotMessage(userId, messageText) {
  ...
  if (context?.step === 'awaitingCancelDate') {
    ...
    const originalList = rawReservations.map(r => {
      ...
      return { reservationId, timeSlot, selectedDate, display };
    });

    console.log('[DEBUG] originalList 構造体:', JSON.stringify(originalList, null, 2));

  cancelContext.set(userId, {
      step: 'awaitingCancelSelection',
      selectedDate,
      originalList: [ { reservationId, timeSlot, selectedDate, display } ] // ← 構造体であること
  });
