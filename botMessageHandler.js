...
async function handleBotMessage(userId, messageText) {
  ...
  if (context?.step === 'awaitingCancelDate') {
    ...
    const structuredList = rawReservations.map(r => ({
      reservationId: r[0],
      date: selectedDate,
      timeSlot: r[3],
      name: r[4],
      note: r[5],
      display: `🕒 ${r[3]}｜👤 ${r[4]}｜📝 ${r[5]}｜予約枠ID: ${r[0]}`
    }));

    cancelContext.set(userId, {
      step: 'awaitingCancelSelection',
      cancelDate: selectedDate,
      originalList: structuredList
    });

    return `🕒 ${selectedDate} のキャンセルしたい予約を番号で選んでください（例：1）\n📋 予約一覧:\n` +
           structuredList.map((r, i) => `${i + 1}. ${r.display}`).join('\n');
  }
  ...
  if (context?.step === 'awaitingCancelSelection') {
    ...
    console.log('[DEBUG] キャンセル対象:', selected);

    const { reservationId, timeSlot, date } = selected;
    ...
    cancelContext.delete(userId);
    return await cancelReservation(userId, reservationId, date, timeSlot);
  }
