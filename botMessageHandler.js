...
async function handleBotMessage(userId, messageText, sendReply) {
  ...
  if (state.step === 'awaitingDate') {
    ...
    console.log(`[DEBUG] getAvailableSlots 処理時間: ${Date.now() - start}ms`);
  ...
  if (state.step === 'awaitingFreeDate') {
    ...
    const start = Date.now();
    const slots = await getAvailableSlots(date);
    console.log(`[DEBUG] getAvailableSlots (空き枠表示) 処理時間: ${Date.now() - start}ms`);
    userContext.delete(userId);
    clearReservationCache();
    return slots.length
