...
function extractTimeSlot(text) {
  const match = text.match(/(\d{1,2}:\d{2})\s*[〜～~\-]\s*(\d{1,2}:\d{2})/u);
  if (match) {
...
if (context?.step === 'awaiting_date') {
  try {
    ...
    console.log('[DEBUG] getReservationsByDate - 件数:', rawList.length);
    if (rawList.length > 0) {
      console.log('[DEBUG] getReservationsByDate - 1件目:', rawList[0]);
    }
    ...
    const reservations = rawList.map((r) => {
      ...
      console.log('[DEBUG] 抽出対象:', r);
      console.log('[DEBUG] 抽出結果 → reservationId:', reservationId, 'timeSlot:', timeSlot, 'date:', date);
    ...
    if (reservations.length === 0) {
      cancelContext.delete(userId);
      return `⚠️ 抽出に失敗しました。予約データの形式をご確認ください。`;
    }
  ...
  if (context?.step === 'awaiting_selection') {
    ...
    if (!selected.reservationId || !selected.timeSlot || !selected.date) {
      console.warn('[WARN] キャンセル対象の情報が不完全です:', selected.display);
