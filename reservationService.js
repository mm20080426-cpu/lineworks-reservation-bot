...
async function getCachedReservations() {
  if (!reservationCache) {
    reservationCache = await readReservationData();
  }
...
/** 🧼 比較用：時間枠の表記を統一（順序変更なし） */
function normalizeSlotForComparison(slot) {
  return slot.replace(/〜|～|~|-/g, '〜').trim();
}
...
async function registerReservation(userId, selectedDate, timeSlot, name, note) {
  ...
  const normalizedSlot = normalizeSlotForComparison(timeSlot);

  const allSlotsRaw = getAvailableTimeSlots(formattedDate);
  const availableSlots = Array.isArray(allSlotsRaw) ? allSlotsRaw.map(normalizeSlotForComparison) : [];
...
async function cancelReservation(userId, reservationId, selectedDate, timeSlot) {
  try {
    ...
    const allReservations = await getCachedReservations();
    ...
    const targetIndex = dataRows.findIndex(row =>
      ...
      normalizeSlotForComparison(row[3]) === normalizedSlot
    );
    ...
    const name = targetRow[4];
    const note = targetRow[5];
...
async function getReservationsByDate(selectedDate) {
  ...
  const sorted = filtered.sort((a, b) =>
    normalizeSlotForComparison(a[3]).localeCompare(normalizeSlotForComparison(b[3]))
  );

  return sorted.map((row, index) => {
    const [reservationId, , , timeSlot, name, note] = row;
    return `${index + 1}. ${normalizeSlot(timeSlot)}｜👤 ${name}｜📝 ${note || 'なし'}｜予約枠ID: ${reservationId}`;
  });
}
...
async function getReservationsByDateRaw(selectedDate) {
  ...
  const dataRows = allReservations.slice(1);
  return dataRows.filter(row => normalizeDate(row[2]) === formattedDate);
}
...
async function isDuplicateReservation(userId, selectedDate, timeSlot) {
  ...
  return dataRows.some(row => {
    ...
    return id === userId &&
           normalizeDate(date) === formattedDate &&
           normalizeSlotForComparison(slot) === normalizedSlot &&
           status === 'reserved';
  });
}
...
async function getAvailableSlots(selectedDate) {
  ...
  const reservedSet = new Set(
    dataRows
      ...
      .map(row => normalizeSlotForComparison(row[3]))
  );

  const allSlotsRaw = getAvailableTimeSlots(formattedDate);
  const allSlots = Array.isArray(allSlotsRaw)
    ? allSlotsRaw.map(normalizeSlotForComparison)
    : [];

  console.log('[DEBUG] reservedSet:', Array.from(reservedSet));
