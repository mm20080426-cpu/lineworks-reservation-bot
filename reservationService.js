...
async function getAvailableSlots(selectedDate) {
 ...
 const ignoreNamesPattern = /(まつもと|松本|マツモト|まさのり|事務長)/;
const ignoreKeywords = ['検証', 'キャンセル', '調整', 'テスト'];

const reservedSet = new Set(
  dataRows
    .filter(row => {
      const dateMatch = normalizeDate(row[1]) === normalizedSelectedDate;
      const note = row[4] || '';
      const name = row[3] || '';
      const isValidNote = !ignoreKeywords.some(keyword => note.includes(keyword));
      const isValidName = !ignoreNamesPattern.test(name);

        // ✅ デバッグログ追加
        if (dateMatch) {
          console.log(`[DEBUG] 判定対象: ${row[2]}｜${name}｜${note}｜isValidNote=${isValidNote}｜isValidName=${isValidName}`);
        }

        return dateMatch && isValidNote && isValidName;
      })
