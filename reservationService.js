...
async function getAvailableSlots(selectedDate) {
  ...
  const reservedSlots = dataRows
    .filter(row => row[1].trim() === formattedDate)
    .map(row => row[2]);

  const allSlots = getAvailableTimeSlots(selectedDate); // ← calendar.json から取得

  if (!Array.isArray(allSlots)) return []; // null や undefined の場合は空配列
