...
const { isValidReservation, getAvailableTimeSlots } = require('./calendarUtils');
const { appendToHistorySheet } = require('./historyWriter'); // ✅ 退避用関数を別ファイルに分離してもOK
...
async function getReservationsByDate(selectedDate) {
  ...
  const filtered = dataRows.filter(row => {
    const sheetDate = String(row[1]).trim().replace(/-/g, '/');
...
/** 指定日付の空き時間枠を取得（calendar.jsonベース） */
async function getAvailableSlots(selectedDate) {
  ...
  // Sheets側の予約済み枠を抽出（selectedDateは "YYYY-MM-DD"）
  const reservedSlots = dataRows
    .filter(row => {
      const sheetDate = String(row[1]).trim().replace(/\//g, '-'); // "2025/09/12" → "2025-09-12"
      return sheetDate === selectedDate;
    })
    .map(row => normalizeSlot(row[2]));

  // calendar.json からその日の全枠を取得
  const allSlotsRaw = getAvailableTimeSlots(selectedDate);
  ...
  console.log('[DEBUG] reservedSlots:', reservedSlots);
