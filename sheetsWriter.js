...
// ✅ 予約データを全件上書き（キャンセル後など）
async function overwriteReservationData(dataArray) {
  try {
    const sheets = await getSheetsClient();
    ...
    console.log('[INFO] 予約データ上書き成功:', dataArray);
  } catch (err) {
    console.error('[ERROR] 予約データ上書き失敗:', err.message);
    throw err;
  }
}
...
async function appendToHistorySheet(rowData) {
  try {
    ...
    if (rowData.length < 9) {
      rowData[8] = new Date().toISOString().split('T')[0];
    }
...
module.exports = {
  ...
  overwriteReservationData,  // 全件上書き（キャンセル後など）
