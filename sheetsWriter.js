...
// 🔍 デバッグログ（認証情報の確認）
console.log('[DEBUG] client_email:', process.env.GS_CLIENT_EMAIL ? 'OK' : '未設定');
console.log('[DEBUG] private_key preview:', process.env.GS_PRIVATE_KEY?.slice(0, 30));
...
async function writeReservationData(dataArray) {
  const spreadsheetId = process.env.GS_SPREADSHEET_ID; // .env から取得
